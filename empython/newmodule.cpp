...
PyObject* newmodule_RegisterFunction(PyObject* pSelf, PyObject* pArgs)
{
  ...
  return NULL;
	...
	for (list<FuncID>::iterator i = FunctionsList.begin(); i != FunctionsList.end(); i++) {
		if ((*i).id == index) {
			// Sorry mates.
	...
	try {
		FunctionsList.insert(FunctionsList.end(), info);
	} catch (...) {
		// Oh uh!
...
{
	DWORD index = 0;
	if (!PyArg_ParseTuple(pArgs, "l", &index)) return NULL;
	...
	for (list<FuncID>::iterator i = FunctionsList.begin(); i != FunctionsList.end(); i++) {
		if ((*i).id == index) {
			...
			PyObject* ret = PyEval_CallObject((*i).cbf, NULL);
			Py_XDECREF(ret);
			Py_INCREF(Py_None);
			return Py_None;
		}
